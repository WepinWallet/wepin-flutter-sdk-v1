...
class WepinWidgetSDK {
  ...
  // StreamController 선언
  final StreamController<Map<String, dynamic>> _wepinEventController = StreamController<Map<String, dynamic>>.broadcast();
  // Stream getter
  Stream<Map<String, dynamic>> get wepinEventStream => _wepinEventController.stream;
  ...
  void _emitLifecycleChange(WepinLifeCycle newValue) {
    if (newValue == WepinLifeCycle.login || newValue == WepinLifeCycle.loginBeforeRegister) {
  ...
  Future<WepinUser> register(BuildContext context) async {
    ...
    if (await getStatus() != WepinLifeCycle.loginBeforeRegister && _userInfo == null) {
      throw WepinError(WepinErrorCode.incorrectLifecycleException, 'The LifeCycle of wepin SDK has to be login_before_register');
    }
    ...
    if (userStatus?.loginStatus == 'registerRequired' &&
      ...
      /// register api
      await _wepinNetwork?.register(RegisterRequest(appId: wepinAppId, userId: userId!, loginStatus: userStatus!.loginStatus, walletId: walletId!));
      await _wepinNetwork?.updateTermsAccepted(userId!, UpdateTermsAcceptedRequest(termsAccepted: ITermsAccepted(termsOfService: true, privacyPolicy: true)));

      await _wepinSessionManager?.wepinStorage.setLocalStorage<WepinUserStatus>('user_status', WepinUserStatus(loginStatus: 'complete'));
      await _checkLoginStatusAndSetLifecycle();
      return _userInfo!;
    } else {
      ...
      _responseEvent[id] = (JSResponse response) async {
        ...
        if (response.body.state == 'SUCCESS') {
          ...
          completer.completeError(
              WepinError(WepinErrorCode.failedRegister, '${response.body.data}'));
        }
        return;
      };
      if (context.mounted) {
  ...
  Future<List<WepinAccount>> getAccounts({List<String>? networks, bool? withEoa}) async {
    if (!_isInitialized) {
      throw WepinError(WepinErrorCode.notInitialized);
    }
    ...
    final userId = _userInfo!.userInfo?.userId;
    final walletId = _userInfo!.walletId;
    final localeId = LocaleMapper.getNumberFromLocale(wepinAttribute.defaultLanguage);

    final accountList = await _wepinNetwork?.getAppAccountList(
        GetAccountListRequest(walletId: walletId!, userId: userId!, localeId: localeId!.toString())
    );
    ...
    if (networks != null && networks.isNotEmpty) {
      return _accountInfo!.where((account) => networks.contains(account.network)).toList();
    }
  ...
  Future<List<WepinAccountBalanceInfo>> getBalance({List<WepinAccount>? accounts}) async {
    if (!_isInitialized) {
      throw WepinError(WepinErrorCode.notInitialized);
    }
    if (await getStatus() != WepinLifeCycle.login && _userInfo == null) {
    ...
    final filteredAccounts = isAllAccounts
        ? _detailAccounts!
        : _detailAccounts!.where((dAccount) => accounts.any((acc) =>
    acc.network == dAccount.network && acc.address == dAccount.address && dAccount.contract == null)).toList();

    if (filteredAccounts.isEmpty) {
      throw WepinError(WepinErrorCode.accountNotFound, 'No matching accounts found');
    }
    ...
    for (var dAccount in filteredAccounts) {
      futures.add(() async {
        var balance = await _wepinNetwork?.getAccountBalance(dAccount.accountId);
        if (balance != null) {
    ...
    if (balanceInfo.isEmpty) {
      throw WepinError(WepinErrorCode.noBalancesFound, 'No balances found for the accounts');
    }
  ...
  WepinAccountBalanceInfo _filterAccountBalance(List<IAppAccount> detailAccounts, IAppAccount dAccount, GetAccountBalanceResponse balance) {
    List<IAppAccount> accTokens = detailAccounts.where((acc) =>
    acc.accountId == dAccount.accountId && acc.accountTokenId != null).toList();
    List<WepinTokenBalanceInfo> findTokens = balance.tokens.isNotEmpty
        ? balance.tokens.where((bal) =>
        accTokens.any((t) => t.contract == bal.contract)).map((x) {
  ...
  Future<List<WepinNFT>> getNFTs({required bool refresh, List<String>? networks}) async {
    if (!_isInitialized) {
      throw WepinError(WepinErrorCode.notInitialized);
    }
    ...
    final GetNFTListResponse? detailNftList = refresh
        ? await _wepinNetwork?.refreshNFTList(GetNFTListRequest(walletId: walletId!, userId: userId!))
        : await _wepinNetwork?.getNFTList(GetNFTListRequest(walletId: walletId!, userId: userId!));
    ...
    final List<IAppAccount> availableAccounts = _detailAccounts!.where((account) =>
    allNetworks || networks.contains(account.network)).toList();
